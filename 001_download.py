...
def get_all_popes():
    ...
    SELECT ?pope ?popeLabel ?birth ?death ?birthPlaceLabel ?startDate ?endDate WHERE {
      ?pope wdt:P39 wd:Q19546.  # position held: pope
...
def save_pope_data():
    ...
    os.makedirs("downloads/popes", exist_ok=True)
    ...
    for i, pope in enumerate(popes):
        ...
        if detailed_data:
            # Save to file
            filename = f"downloads/popes/{entity_id}_{pope_label.replace(' ', '_')}.json"
    ...
    with open("downloads/popes/all_popes_summary.json", "w", encoding="utf-8") as f:
